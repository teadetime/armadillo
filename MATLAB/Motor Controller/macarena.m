...
for ii = 1:length(Pxs) - 1
   Px((ii - 1) .* nSteps + 1:ii .* nSteps) = linspace(Pxs(ii), Pxs(ii + 1), nSteps);
   Py((ii - 1) .* nSteps + 1:ii .* nSteps) = linspace(Pys(ii), Pys(ii + 1), nSteps);
   Pz((ii - 1) .* nSteps + 1:ii .* nSteps) = linspace(Pzs(ii), Pzs(ii + 1), nSteps);
end
