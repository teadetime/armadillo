...
T3 = asin(Py ./ sqrt(Px .^ 2 + Py .^ 2)); % Base lateral
Angle = table(T1, T2, T3);

X1 = L1 .* sin(Angle.T1) .* cos(Angle.T3);
Y1 = L1 .* sin(Angle.T1) .* sin(Angle.T3);
Z1 = L1 .* cos(Angle.T1);

X2 = X1 + L2 .* sin(-Angle.T2) .* cos(Angle.T3); % may later be T4
Y2 = Y1 + L2 .* sin(-Angle.T2) .* sin(Angle.T3); % may later be T4
